...
hxsl 主要适用于 flash 的 AGAL, http://old.haxe.org/manual/hxsl, 你应该多参考 hxsl 库的示例
...
目前还没有文档介绍这些， 因此不保证下边内容的正确性.

* `@local`: 局部变量?

* `@global`: 可以在 shader 方法内部访问，

  > 例: Base2d 中的一个 `@global var time:Float;`, 示例并没有给这个 time 赋值, 但它却有值

* `@var`:

* `@param`: 估计就是 flash-hxsl 中 vertex() 的参数, 因此可以像类普通的成员一样直接访问和赋值,

  > 估计和 flash-hxsl 一样, fragment() 内访问的类型仅能为 Sampler2D, 其它则 vertex()中赋值然后在 fragment()中读取
  >
  > 这个变量值可以为 null(空), 而 @const 则必须提供

* `@const`: 形为和 @param 一样, 只是它是必须的参数不可为空(null)

* `@input`: 标记为 input, 对应于"属性寄存器"
