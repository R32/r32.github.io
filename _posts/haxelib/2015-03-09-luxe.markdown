...
flow 是一个项目构建工具,基于nodejs, flow的安装目录包含有一个nodejs. 通过解析一个 json 配置文件来布署应用.

虽然 flow并非只作为 luxe的构建工具,但是我很怀疑它还能做别的什么, 而且使用源生JS所写
...
```js
{
  project : {
	...
	*/
		  ...
		  */
		  ...
		  android : {
			...
			*/
			...
			/**
			installLocation in android manifest: http://developer.android.com/guide/topics/manifest/manifest-element.html#install
			...
			*/
			...
			/**
			sdk min in android manifest: http://developer.android.com/guide/topics/manifest/uses-sdk-element.html
			...
			*/
			...
			/**
			sdk target in android manifest: http://developer.android.com/guide/topics/manifest/uses-sdk-element.html
	...
	dest: 目标路径则以 output/target/ 为根目录.(位于 project/build/下的files 则以 output/target.build/为根目录)
