...
* [如何生成 Haxe API 文档]({% post_url 2014-05-05-doc-gen %})
...
cppast                 : Generate experimental cpp code
...
# 禁用优化器(haxe 3.3 analyzer 已经是默认打开)
no-analyzer               : Disable the static analyzer
...
## 优化配置 文档 # 参数 https://github.com/HaxeFoundation/haxe/blob/development/analyzer.ml
...
# 见 https://gist.github.com/skial/c1e35988b853b214da39
...
# haxe 3.2 Dev版, 允许类型参数接受表达式， 注意可能需要添加 @:generic
...
# 如果一个字段或类, 被添加这个标记, 编译器将在编译时输出警告信息（注:目前仅适用于hxjava,未来可能会扩展到所有平台）
@:deprecated         : Automatically added by -java-lib on class fields
                     annotated with @Deprecated annotation.
                     Has no effect on types compiled by Haxe.(java only)
...
# **常用于**.当 new T()时, 需要添加这个标记  用于替换掉需要接口 haxe.rtti.Generic类,用于声明泛型类. (since 3.0.
# 标记“类”或“静态字段”为泛型, 因此每个类型参数将分别构建自身
# 比如 Some<Int>,Some<Float> 加上这个元数据后会被分别编译为 Some_Int,和 Some_Float 类
# 对于静态平台来说, 加上这个是最好, 但如果是 JS 平台, 则不需要加
...
#@:initPackage       : 用于 js 平台的 extern class, 用于将 包名 初使为 Object,（已移除）
...
@:phpConstants       : Marks the fields of an TAbstract as php constants,
                        without $ (php only)
