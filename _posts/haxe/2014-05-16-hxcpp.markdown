...
# 但是编译后在 build.xml 相关的 <file /> 元素上仅仅加上一条　<depend />　根本不起作用, 
# 也就是说加了<depend /> 的 cpp 但却无法引用到 depend 的定义, 是个 Bug 吗?　[haxe 3.2 rc2.0]
 ...
 * **cpp.NativeArray** C 语言中所描述的数组, 即同类型连续排列的内存引用
  ...
  - hxarray.(__unsafe_get,__unsafe_set,__get,__set) 带 unsafe 的方法没有做边界检测.
	...
	var pa:cpp.Pointer<Int> = cpp.NativeArray.address([1, 2, 3], 0); // 或者 Pointer.arrayElem([1,2,3], 0 )
...
<files id="__main__">														
  ...
  <precompiledheader name="" dir="" />  <!-- TODO: 预编译,  -->
...
<compiler id="android-gcc" exe="g++">
 ...
 <objdir value="" />			<!-- 指定编译中间文件时的目录, 默认为 obj -->
 <outflag value="" />			<!-- 输出　编译标记, 通常为 -o 或 --output -->
 <pch value="" />				<!-- TODO: 未知 - if(pch == "gcc"){...} -->
 <getversion value="" />		<!-- TODO: 未知 - 示例上看 value 和 exe 元素的基本一样 -->
...
<linker id="dll" exe="link.exe" if="windows">
  ...
  <lib name="${dll_import_link}" if="dll_import_link" />		<!-- 依赖的链接库 -->
...
<prelinker id="" exe="">		<!-- 预链接器 -->
  <exe name="" />				<!-- 执行 预链接的 命令名称 -->
  <flag value="" />		
  <outflag value="" />			<!-- 文件输出 标记,默认为 -o -->
  <expandAr value="" />			<!-- 未知. 默认为 false. -->
  <fromfile value="" />			<!-- 未知. 默认为 @ -->
  <section></section>
</prelinker>

<!-- 目标 -->
  <!-- id: 标识符 -->
  <!-- tool: 似乎只有 linker 这一个值,   -->
  <!-- toolid: id值用于链接器选择, 编译顺序从 prelinker 到 linker  -->
  <!-- output:  输出文件名称,不带扩展名.-->
  <!-- overwrite: 重写, 自已看源码定义 -->
<target id="msvccompat" output="" tool="linker" toolid="${STD_MODULE_LINK}" >
  <target id="" />				<!-- 子目标 -->
  <lib name="some.lib" />		<!-- 库名  -->
  <flag />						<!--  -->
  <depend />					<!--  -->
  <vflag />						<!--  -->
  <dir />						<!--  -->
  ...
  <builddir name="" />	<!--  -->
  <ext />				<!--  --> 
  <files id="__main__" />		<!-- 选择分组, 不同 id 表示不同分组 -->
  <section></section>
</target>
```
