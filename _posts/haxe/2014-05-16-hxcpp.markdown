...
* TIPS: 目前可以用 `-D cppia` 来调试, 实际上目前可以指定编译目标为 cppia, 但是 IDE 似乎并不支持

  > 下边附录部分有关于如何使用 cppia 的介绍
...
# TODO: 估计是用于将 haxe 的类导出为外部类给 C++ 调用, 通常你可能需要使用 -D static_link 来定义为静态链接库
@:nativeGen				#
...
一些定义你可以在 `hxcpp/toolchain` 目录下的 xml 文件中找到.

```bash
HAXE_OUTPUT_FILE      # 指定输出文件名称, 因为 -cpp bin 指定的是个目录

static_link           # 生成成静态库, 而不是 exe 文件
