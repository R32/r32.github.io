...
[hxcpp-guide](https://github.com/snowkit/hxcpp-guide) 手册进度未知.

* 以 extern 的形式编译第三方库 <http://snowkit.github.io/linc/>, 这种形式避免是直接使用 c/cpp 源码, 因此更快
...
这里主要记录一些和 hxcpp 相关的东西, 由于没有找到 hxcpp 的相关文档, 因此不保证内容的正确性.
...
首先我们需要知道在 haxe 这边调用 `new Foo()` 这时相应的 cpp 代码会做些什么？
...
首先，我们可以看到 haxe 中的 Foo 编译后变成了 Foo_obj， 其次， `new` 变成了 `__new` 方法
...
> 由于在 haxe 代码中通常不会有类似于 `c/c++` 的 malloc/free 之类的操作，因此 hxcpp 被设计成一个带有 GC 的框架
>
> hxcpp 为了支持 haxe 中的 反射(Reflect)，因此会添加一此额外的方法，不过可使用 `@:unreflective` 关闭这一项
>
> 可以不要 GC 和 Reflect 吗?, 当然如果你将 `@:nativeGen` 添加到类或接口上，则代码看上去非常干净，但是这样你将无法从 haxe 这边使用 new 关键字，因为 new 是和 GC 一起的
...
* 外部类由于并非从 GC 申请， 因此需要自已管理内存
  ...
  如果你像上边返回的是 Pointer 对象， 那么可以调用 `Pointer::destroy 与 destroyArray`, 它们分别对应 `delete ptr 和 delete [] ptr`
...
> 提供一个静态方法 fromStaticFunction 能简单创建 cpp.Callable，但怪异的是那个方法体的内部得是 `__cpp__` 代码才行
