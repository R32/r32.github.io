...
* c# 可使用 struct 用于创建紧凑数据, haxe 没有相关功能，
  ...
  > 虽然 haxe 中的"匿名结构"看上去非常像 struct , 但"匿名结构"是一种降低性能的 Dynamic 结构数据，
  > haxe 中的"匿名结构"只对 js 和 neko 这二个平台没有性能影响
...
而 haxe 中定义 interface 则可以有字段, 但是在 haxe 中 `var id:Int; 与 var id(default, null):Int;`
并不是同一种东西, 前者称为 field(字段), 后者为 property(属性).
