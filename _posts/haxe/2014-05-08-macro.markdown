...
// 使用条件运算符检测, 注意条件运符需要使用“小括号”
#if(haxe_ver > 3.2)
  trace("haxe version > 3.2");
#end

// 支持逻辑运算符：&& 和 || ,和条件运算符一样, 同样需要有“小括号”
#if (neko && debug)
...
需要注意的是假如你定义了 `-D some-flag`, 那么在检测时应该把减号 "-" 替换成 "_", 因此应该使用 `#if some_flag` 来检测。

再重复的就是如果使用 条件运算符,或逻辑运算符 那么表达式一定记得使用小括号。例: `#if (haxe_ver >= 3.3)`

> 如果你的项目是 openfl 项目, 那么通常你应该修改 xml 配置文件来设置 Defines, 如: `<haxedef name="hello" value="world" />`

#### require

同样可以把条件标识符放置在 `@:require` 之后，如果没有满足条件, 那么类名可以访问, 但是类的所有字段(包括static类型)都不可访问.
