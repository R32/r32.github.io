...
**注意**: 宏方法 assign 和 noChange 只有返回值不一样, 充分说明了 宏返回值 将替换宏调用. 这一概念很重要.
 ...
 * 限制: haxe 3.2 注意当宏方法为成员方法(非static)时, 不能在同一个相同文件中调用这个宏方法。
...
将常量作为类型传递, http://haxe.org/manual/macro-generic-build.html#const-type-parameter
