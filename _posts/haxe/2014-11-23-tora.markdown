...
由于依赖于 apache/nginx, 因此也只适用于作为 HTTP 服务, 其它的例如 WebSocket 的实时交互还是得另外写后台。
...
**建议使用 apache** 作为服务器, 因此 nginx 的 fcgi 模式是其它人提供的 patch, 稳定性未知, 一些功能感觉有却失, 虽然我一开始使用的是 nginx 但最终更改回 apache
...
-unsafe <host:port>	# 打开一个端口，允许直接连接到 Tora 服务器. 而不是通过 apache 代理
 ...
 * 选择 httpd.apache 2.2 版本(担心与2.4不太兼容), 这里选的是 [windows 平台](http://httpd.apache.org/docs/current/platform/windows.html#down), 然后根据 readme 文件安装
 ...
 * 在 tora 服务器端输出 log 时用 Sys.stderr() 或 Sys.stdout()

 * Protocol + Queue 用于实现实时消息推送（comet）
 ...
 * `getExports(host: String):Dynamic;` 与 VirtualHost 相关文件Module的 exportsTable
 ...
 * 因此 Protocol 仅仅是让 flash 从 tora服务器像http协议那样获得数据, 并不能交互。
	...
	> 实际测试, 当tora有了二个阻塞的index.n线程后, 不会再新建线程并且会使 tora 被阻塞, 直至前边二个index.n线程释放。
	
	> **更新**:, Queue 的 AddHandler 可以防止socket连接中断而保持长连接,但又会阻塞index.n.
...
```haxe
class Main extends tora.Protocol{
	static function main() {
		var h = "http://localhost:6667";
		trace("connect to:" + h);
		var m = new Main(h);
		m.addHeader("headerKey", "headerValue");
		m.addParameter("paramKey", "paramValue");
		m.connect();
	}
	...
	override public function onError(msg:String) {
		trace("error: " + msg);
	}

	override public function onDisconnect() {
		trace("disconnect");
	}
}
```
...
class Main{
	static public function main(){
		Sys.println("ClientHeaders: " + neko.Web.getClientHeaders());
		Sys.println("ClientParams" + neko.Web.getParams());
...
class Queue<T> {
    ...
    function addHandler( h : Handler<T> );	// 调用后防止与 Protocol 断开连接,即使 index.n 已经退出. 这样才能实现消息推送。
 ...
 * 感觉 nginx + fcgi 好像有问题(未知)

 * 如何实现高效的 session, 感觉使用 Share 模块并不太好，因为感觉 Share 的数据不适合高频率的访问

 * tora 是如何实现将 haxe 生成的 neko 代码被另一个 neko 模块用 Lib.load 来调用???
