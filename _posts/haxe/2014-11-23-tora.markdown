...
```bash
> neko tora.n -unsafe localhost:6667 -config vh.conf
...
class Main{
	static function main() {
		...
		trace("connect to:" + h);
		var p = new tora.Protocol(h);
		
		p.onData = function(data:String){
			trace("data: " + data);
		};
		
		p.onError = function(msg:String){
			trace("error: " + msg);
		};
		...
		p.addHeader("headerKey", "headerValue");
		p.addParameter("paramKey", "paramValue");
		p.connect();
	}
}
```
 ...
 * A-client 通过 addHandler 监听服务器消息推送（Socket 长连接）, 
	...
	p.reset();	// 重置所有请求参数, 但是要注意当出现 disconnect 后重新连接的参数
...
class Test {
	...
	static function main() {
		...
		p.onDisconnect = function() {
			connected = false;
			trace("DISCONNECTED");
			//p.reset(); p.addParameter("wait","1"); p.connect();
		};
