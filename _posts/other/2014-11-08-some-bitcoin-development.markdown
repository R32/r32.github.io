 ...
 * transaction fee 或称为 miner fee,所有剩余的钱,当全部收入交易扣除全部支出交易, 该费用交付给将交易添加到 block 的矿工
	
	> 注意区别 Minimum relay fee(最低中转费): 最低费用交易必须被接入到 内存池通过 Bitcoin Core 节点中转

#### Proof Of Work

块链由网络匿名节点共同协作维护, 所以比特币要求创建的每一个block证明其工作量投入,以确保不受信任的其它人想要修改历史的工作难度要大于添加新block到块链的诚实点.

将block连接在一起使得交易不能被修改,并且无需修改后续block. 其结果是想要修改某一特定block的工作量代价随着每个新block添加到块，而随之放大.
...
在上面给出的示例中, 你可能在第一次尝试中就产生出成功的哈希值,你甚至可以预计一个给定目标阈值的哈希的尝试次数, 比特币假定一个线性概率, 目标阈值越低, 则需要更多次哈希计算尝试.
